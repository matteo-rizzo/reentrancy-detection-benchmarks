...
contract dexBlue {
    ...
    function multiSigWithdrawal(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked( // Calculate the withdrawal hash from the parameters
    ...
    function multiSigTransfer(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked( // Calculate the withdrawal/transfer hash from the parameters
                    "\x19Ethereum Signed Message:\n32",
                    keccak256(
                        abi.encodePacked(
                            msg.sender,
    ...
    function userSigWithdrawal(
        ...
        bytes32 hash;
        if (v < 30) {
            // Standard signing scheme (personal.sign())
            hash = keccak256(
                abi.encodePacked( // Restore multi-sig hash
                        "\x19Ethereum Signed Message:\n32",
                        keccak256(
                            abi.encodePacked(
                                token,
    ...
    function blockFundsForSingleSigWithdrawal(
        ...
        if (
            ...
            // Check if the user holds the required funds
    ...
    function matchTrades(
        ...
        for (uint8 i = 0; i < len; i++) {
            // Loop through the orders array to restore all signees
            ...
            if (order.v < 0) {
                // Check if the order is signed by a delegate
                delegated = true;
            ...
            if (order.v < 30) {
                ...
                hashes[i] = keccak256(
                    abi.encodePacked( // Restore the hash of this order
                            "\x19Ethereum Signed Message:\n32",
                            keccak256(
                                abi.encodePacked(
                                    addressCache1,
        ...
        for (i = 0; i < len; i++) {
            // Loop through trades to settle after checks
            ...
            if (
                // Check if the arbiter has matched following the conditions of the two order signees
                ...
                (orders[maker_index].sell_amount * trade.taker_amount) /
                    ...
                    1 && // Check maker doesn't overpay (+ 1 to deal with rouding errors for very smal amounts)
                trade.taker_amount <=
                (orders[taker_index].sell_amount * trade.maker_amount) /
                    orders[taker_index].buy_amount +
                    1 && // Check taker doesn't overpay (+ 1 to deal with rouding errors for very smal amounts)
                ...
                balances[addressCache2][signee[maker_index]] -=
                    trade.maker_amount -
                    trade.maker_rebate; // Substract maker's sold amount minus the makers rebate
                balances[addressCache1][signee[taker_index]] -= trade
                    .taker_amount; // Substract taker's sold amount

                // Add bought amounts
                balances[addressCache1][signee[maker_index]] +=
                    trade.taker_amount -
                    trade.maker_fee; // Give the maker his bought amount minus the fee
                balances[addressCache2][signee[taker_index]] +=
                    trade.maker_amount -
                    trade.taker_fee; // Give the taker his bought amount minus the fee
                ...
                balances[addressCache2][feeCollector] +=
                    trade.taker_fee -
                    trade.maker_rebate; // Give the feeColletor the taker fee minus the maker rebate
                ...
                blocked_for_single_sig_withdrawal[addressCache2][
                    signee[maker_index]
                ] = 0; // If the maker tried to block funds which he/she used in this order we have to unblock them
                blocked_for_single_sig_withdrawal[addressCache1][
                    signee[taker_index]
                ] = 0; // If the taker tried to block funds which he/she used in this order we have to unblock them
        ...
        if (
            arbiters[ // Check if the signee is an arbiter
                ecrecover( // Restore the signing address
                    keccak256(
                        abi.encodePacked( // Restore the signed hash (hash of all orderHashes)
                                "\x19Ethereum Signed Message:\n32",
    ...
    function revokeDelegation(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked( // Restore the signed hash
                    "\x19Ethereum Signed Message:\n32",
                    keccak256(
    ...
    function checkERC20TransferSuccess() private pure returns (bool) {
        ...
        assembly {
            ...
            case 0 {
                // Nothing returned, but contract did not throw > assume our transfer succeeded
                success := 1
            }
            case 32 {
                // 32 bytes returned, result is the returned bool
